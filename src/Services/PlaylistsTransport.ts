...
export class PlaylistsTransport extends Transport {
    static getCurrentUserPlaylists(limit = 20, offset = 0) {
        return this.get(`/me/playlists?limit=${limit}&offset=${offset}`, json => json as ISpotifyPlaylistsInfo);
    }

    static getUserPlaylists(userId: string, limit = 20, offset = 0) {
    ...
    static createPlaylist(userId: string, name: string, isPublic: boolean = true, isCollaborative: boolean = false, description: string = '') {
    ...
    static getPlaylistTracks(playlistId: string, limit = 20, offset = 0) {
        return this.get(`/playlists/${playlistId}/tracks?limit=${limit}&offset=${offset}`, json => json as ISpotifyTracksInfoWithDate);
    }

    static addTracksToPlaylist(playlistId: string, uris: string[], position: number = 0) {
    ...
    static deleteTracksFromPlaylist(playlistId: string, uris: string[]) {
